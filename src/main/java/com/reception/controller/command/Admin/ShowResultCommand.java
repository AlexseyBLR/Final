...
public class ShowResultCommand implements Command {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException, DAOException {
        ...
        List<ResultForRequest> listME = resultService.getResult(ResultProperty.MECHANICAL_EXPLOATATION_FACULTY);
        request.getSession().setAttribute("resultME", listME);

        List<ResultForRequest> listMR = resultService.getResult(ResultProperty.MECHANICAL_REPAIR_FACULTY);
        request.getSession().setAttribute("resultMR", listMR);

        List<ResultForRequest> listAD = resultService.getResult(ResultProperty.ARCHITECTURE_DESIGN_FACULTY);
        request.getSession().setAttribute("resultAD", listAD);

        List<ResultForRequest> listAB = resultService.getResult(ResultProperty.ARCHITECTURE_BUILDING_FACULTY);
